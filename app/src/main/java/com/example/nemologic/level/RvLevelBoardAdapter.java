...
public class RvLevelBoardAdapter extends RecyclerView.Adapter<RvLevelBoardAdapter.ViewHolder> {
    ...
    public RvLevelBoardAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        offset += parent.getMeasuredWidth() % this.width;
        if(offset >= 1)
        {
            view.setLayoutParams(new RecyclerView.LayoutParams(parent.getMeasuredWidth()/this.width + 1, parent.getMeasuredHeight()/this.height + 1));
            offset--;
        }
        else
        {
