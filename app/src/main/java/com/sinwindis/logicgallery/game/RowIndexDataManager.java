...
public class RowIndexDataManager {
    ...
    public void makeIdxDataSet() {
        int maxIdxNum = lpm.getWidth() / 2 + 1;
        idxDataSet = new int[lpm.getHeight()][maxIdxNum];
        idxNumSet = new int[lpm.getHeight()];
        int sumTemp;
        int idxNum;
    ...
    public boolean[] getIdxMatch(int rowNum) {
        ...
        for (int i = 0; i < lpm.getWidth(); i++) {
            lastCheckIdx = i;
            switch (lpm.getCell(rowNum, i).getCurrentValue()) {
                case 0:
                    //만약 공백이 나오면
            ...
            //반복문을 끝낸다.
            if (checkIdx == idxNum) {
                for (int j = i + 1; j < lpm.getWidth(); j++) {
                    if (lpm.getCell(rowNum, j).getCurrentValue() == 1) {
                        //체크된 부분이 있다면
        ...
        for (int i = lpm.getWidth() - 1; i > lastCheckIdx; i--) {
            switch (lpm.getCell(rowNum, i).getCurrentValue()) {
                case 0:
                    //만약 공백이 나오면
                    //개수가 맞는지 체크하고 맞건 안맞건 반복문을 끝낸다.
        ...
        for (int i = 0; i < lpm.getWidth(); i++) {
            if (lpm.getCell(rowNum, i).getCurrentValue() == 1) {
                //해당 칸이 채워져있을때
                sumTemp++;
            } else {
                //해당 칸이 채워져있지 않을 때
            ...
            if (sumTemp == idxDataSet[rowNum][checkIdx]) {
                ...
                if (i == lpm.getWidth() - 1 || lpm.getCell(rowNum, i + 1).getCurrentValue() != 1) {
